...
MainWindow::MainWindow(GtkApplication* app) : scene(new Scene()), draggingLight(false), lightPlaneY(3.0f) {
...
gboolean MainWindow::on_motion_notify(GtkWidget* widget, GdkEventMotion* event, gpointer data) {
    ...
    // Only move the light while dragging it
    if (!mw->draggingLight) return TRUE;
    if (!(event->state & GDK_BUTTON1_MASK)) {
        mw->draggingLight = false;
        return TRUE;
    }
    ...
    float worldX, worldZ;
    if (unprojectScreenToFloor(screenX, screenY, mw->lightPlaneY, w, h, worldX, worldZ)) {
        mw->scene->setLightWorldPos(worldX, mw->lightPlaneY, worldZ);
        gtk_widget_queue_draw(mw->gl_area);
    }
...
gboolean MainWindow::on_button_press(GtkWidget* widget, GdkEventButton* event, gpointer data) {
    ...
    if (isLightMode) {
        // In Light mode: place/move the single light to click position and start dragging
        float worldX, worldZ;
        if (unprojectScreenToFloor(screenX, screenY, mw->lightPlaneY, w, h, worldX, worldZ)) {
            mw->scene->setLightWorldPos(worldX, mw->lightPlaneY, worldZ);
            mw->draggingLight = true;
            gtk_widget_queue_draw(mw->gl_area);
        }
    } else {
        // In Cube mode: place a cube on the floor
        GdkRGBA color;
        gtk_color_chooser_get_rgba(GTK_COLOR_CHOOSER(mw->color_button), &color);
